...
import '../../widgets/submission_input_card.dart';
...
class _QuestSubmissionScreenState extends State<QuestSubmissionScreen> {
  ...
  void _share() {
    showModalBottomSheet(
      ...
      builder: (_) => SharePreviewSheet(
        quest: widget.quest,
        timeSpent: _timeController.text.trim().isEmpty
            ? null
            : _timeController.text.trim(),
        where: _whereController.text.trim().isEmpty
            ? null
            : _whereController.text.trim(),
        emotions: _selectedEmotions.map((i) => _emotions[i].label).toList(),
      ),
    );
  }
  ...
  Widget _buildPhotoSection() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const SectionHeader(label: 'PHOTO'),
  ...
  Widget _buildWhatSection() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const SectionHeader(label: 'WHAT DID YOU DO'),
        const SizedBox(height: AppSpacing.md),
        Container(
          decoration: BoxDecoration(
            color: AppColors.bgSurface,
      ...
      children: [
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const SectionHeader(label: 'TIME SPENT'),
              const SizedBox(height: AppSpacing.md),
              SubmissionInputCard(
        ...
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const SectionHeader(label: 'WHERE'),
              const SizedBox(height: AppSpacing.md),
              SubmissionInputCard(
  ...
  Widget _buildEmotionsSection() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const SectionHeader(label: 'HOW DID YOU FEEL'),
        const SizedBox(height: AppSpacing.md),
        SizedBox(
          height: 90,
