...
import '../../../core/services/ai/ai_quest_context.dart';
...
class QuestsController extends ChangeNotifier {
  ...
  String? activeQuestCharacterName;
      ...
      if (doneToday) {
        ...
        if (existing != null && existing.expiresAt.isAfter(DateTime.now())) {
          ...
          activeQuestCharacterName =
              await _resolveCharacterName(existing.characterId);
          _startExpiryTimer();
        } else {
          if (existing != null) {
  ...
  Future<void> activateBreakMode() async {
    ...
    activeQuest = null;
    activeQuestCharacterName = null;
    if (!_disposed) notifyListeners();
  }

  Future<void> activateBreakModeManually() async {
  ...
  Future<void> declineBreakOffer() async {
    ...
    if (pendingChallengeOffer == null) await _checkChainOffer();
  }
  ...
  Future<void> debugTriggerChainOffer({int chainTotal = 2}) async {
    pendingChainOffer = null;

    final category = currentBalance?.category ?? 'physical';
    final nature = currentBalance?.nature ?? QuestNature.action;
  ...
  Future<String?> _resolveCharacterName(int? characterId) async {
    if (characterId == null) return null;
    final all = await _isar.characters.where().findAll();
    try {
      return all.firstWhere((c) => c.id == characterId).name;
    } catch (_) {
      return null;
    }
  }
  ...
  Future<GeneratedQuest> _generateQuest({
    required CharacterDefinition definition,
    ...
    if (EntitlementService.isPro) {
      ...
      if (online) {
        final aiResult = await AIQuestService.generate(
          model: AIConfig.activeModel,
          context: AIQuestContext(
            characterName: definition.name,
            characterEpithet: definition.epithet,
            category: category,
            nature: nature ?? QuestNature.action,
            recentTitles: recentTitles,
          ),
        );
        if (aiResult != null) {
          if (kDebugMode) debugLastQuestSource = 'AI · ${AIConfig.activeModel.displayName}';
          return aiResult;
        }
        if (kDebugMode) debugLastQuestSource = 'Procedural (AI failed)';
      } else {
        if (kDebugMode) debugLastQuestSource = 'Procedural (offline)';
      }
    } else {
      if (kDebugMode) debugLastQuestSource = 'Procedural (not Pro)';
    }
  ...
  Future<void> _assignNewQuest(
      {List<String> excludeRequires = const []}) async {
    final completed = await _loadStats();
    ...
    final definition = result.character!;
    final generated = await _generateQuest(
      definition: definition,
      category: result.category,
      nature: result.nature,
      recentTitles: completed.take(5).map((q) => q.title).toList(),
      excludeRequires: excludeRequires,
    );
    ...
    activeQuest = quest;
    activeQuestCharacterName = isarCharacter?.name;
